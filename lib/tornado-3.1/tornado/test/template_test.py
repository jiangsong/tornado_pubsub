...
""",
    ...
    def test_unicode_literal_expression(self):
        ...
        # template.py file (with utf8 encoding), i.e. \u escapes would not
        # be used in the template.py file itself.
        if str is unicode_type:
            # python 3 needs a different version of this test since
            # 2to3 doesn't run on template.py internals
            template = Template(utf8(u('{{ "\u00e9" }}')))
        else:
...
{% block name %}base: {{ name }}{% end %}""",
            ...
            # Extend a base template.py with different autoescape policy,
