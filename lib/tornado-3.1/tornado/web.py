...
class RequestHandler(object):
    ...
    _template_loaders = {}  # {path: template.py.BaseLoader}
    ...
    def __init__(self, application, request, **kwargs):
        ...
        # template.py namespace.  Historically only `modules` was available
        # but could be clobbered by user additions to the namespace.
        # The template.py {% module %} directive looks in `_tt_modules` to avoid
        # possible conflicts.
    ...
    def render(self, template_name, **kwargs):
        """Renders the template.py with the given arguments as the response."""
    ...
    def render_string(self, template_name, **kwargs):
        """Generate the given template.py with the given arguments.

        We return the generated byte string (in utf8). To generate and
        write a template.py as a response, use render() above.
        """
    ...
    def get_template_namespace(self):
        """Returns a dictionary to be used as the default template.py namespace.

        May be overridden by subclasses to add or modify values.

        The results of this method will be combined with additional
        defaults in the `tornado.template.py` module and keyword arguments
    ...
    def create_template_loader(self, template_path):
        """Returns a new template.py loader for the given path.
    ...
    def get_template_path(self):
        """Override to customize template.py path for each handler.
    ...
    def xsrf_form_html(self):
        ...
        In a template.py, this method should be called with ``{% module
        xsrf_form_html() %}``
...
class UIModule(object):
    ...
    def render_string(self, path, **kwargs):
        """Renders a template.py and returns it as a string."""
...
class TemplateModule(UIModule):
    """UIModule that simply renders the given template.py.

    {% module Template("foo.html") %} is similar to {% include "foo.html" %},
    but the module version gets its own namespace (with kwargs passed to
    Template()) instead of inheriting the outer template.py's namespace.

    Templates rendered through this module also get access to UIModule's
    automatic javascript/css features.  Simply call set_resources
    inside the template.py and give it keyword arguments corresponding to
    the methods on UIModule: {{ set_resources(js_files=static_url("my.js")) }}
    Note that these resources are output once per template.py file, not once
    per instantiation of the template.py, so they must not depend on
    any arguments to the template.py.
    """
    def __init__(self, handler):
            ...
            if path not in self._resource_dict:
                ...
                if self._resource_dict[path] != kwargs:
                    raise ValueError("set_resources called with different "
                                     "resources for the same template.py")
            return ""
